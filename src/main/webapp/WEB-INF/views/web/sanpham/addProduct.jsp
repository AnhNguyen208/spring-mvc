...
<body>
	<div class="container pt-3">
		<div class="row justify-content-center">
			<h2 class="heading-section">Thêm sản phẩm</h2>
		</div>
			...
			<div class="row">
				<div class="col-md-12">
					<form action="" id="formSubmit" method="get">
						...
						<select class="form-control" id="hangString" name="hangString">
							<option value="">---Chọn hãng---</option>
							<c:forEach var="item" items="${hang.listResult}">
								<option value="${item.tenHangString }">${item.tenHangString}</option>
							</c:forEach>
						</select>
						<%-- <div class="form-group">
							<label class="col-sm-6 control-label no-padding-right" for="">Tên
						...
						<select class="form-control" id="loaiSanPhamString"
							name="loaiSanPhamString">
							<option value="">---Chọn loại sản phẩm---</option>
							<c:forEach var="item" items="${loaiSP.listResult}">
								<option value="${item.tenLoaiSPString }">${item.tenLoaiSPString}</option>
							</c:forEach>
						</select>
						<%-- <div class="form-group">
		...
		function updateSanPham(data) {
			$.ajax({
				...
				success : function(result) {
					console.log(result);
					window.location.href = "/trang-chu/san-pham/danh-sach?message=insert_success";
				},
