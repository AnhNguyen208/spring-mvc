package com.quanlydoanhthu.dto;

public class SanPhamDTO extends AbstractDTO<SanPhamDTO>{
	private String tenSanPhamString;
	private String anhDaiDienString;
	private String loaiSanPhamString;
	private String hangString;
	private String logoHangString;
	private Long giaBanLong;
	private Long giaNhapLong;
	private Long giaBanLong1;
	private Long giaNhapLong1;
	private int soLuong;
	private int soLuongBan;
	private int soLuongNhap;
	private String mauSacString;
	private String kichCoString;
	
	public String getLogoHangString() {
		return logoHangString;
	}
	public void setLogoHangString(String logoHangString) {
		this.logoHangString = logoHangString;
	}
	public Long getGiaBanLong1() {
		return giaBanLong1;
	}
	public void setGiaBanLong1(Long giaBanLong1) {
		this.giaBanLong1 = giaBanLong1;
	}
	public Long getGiaNhapLong1() {
		return giaNhapLong1;
	}
	public void setGiaNhapLong1(Long giaNhapLong1) {
		this.giaNhapLong1 = giaNhapLong1;
	}
	public String getMauSacString() {
		return mauSacString;
	}
	public void setMauSacString(String mauSacString) {
		this.mauSacString = mauSacString;
	}
	public String getKichCoString() {
		return kichCoString;
	}
	public void setKichCoString(String kichCoString) {
		this.kichCoString = kichCoString;
	}
	public String getHangString() {
		return hangString;
	}
	public void setHangString(String hangString) {
		this.hangString = hangString;
	}
	public Long getGiaNhapLong() {
		return giaNhapLong;
	}
	public void setGiaNhapLong(Long giaNhapLong) {
		this.giaNhapLong = giaNhapLong;
	}
	public int getSoLuongNhap() {
		return soLuongNhap;
	}
	public void setSoLuongNhap(int soLuongNhap) {
		this.soLuongNhap = soLuongNhap;
	}
	public int getSoLuongBan() {
		return soLuongBan;
	}
	public void setSoLuongBan(int soLuongBan) {
		this.soLuongBan = soLuongBan;
	}
	public String getLoaiSanPhamString() {
		return loaiSanPhamString;
	}
	public void setLoaiSanPhamString(String loaiSanPhamString) {
		this.loaiSanPhamString = loaiSanPhamString;
	}
	public int getSoLuong() {
		return soLuong;
	}
	public void setSoLuong(int soLuong) {
		this.soLuong = soLuong;
	}
	public String getTenSanPhamString() {
		return tenSanPhamString;
	}
	public void setTenSanPhamString(String tenSanPhamString) {
		this.tenSanPhamString = tenSanPhamString;
	}
	public String getAnhDaiDienString() {
		return anhDaiDienString;
	}
	public void setAnhDaiDienString(String anhDaiDienString) {
		this.anhDaiDienString = anhDaiDienString;
	}
	public Long getGiaBanLong() {
		return giaBanLong;
	}
	public void setGiaBanLong(Long giaBanLong) {
		this.giaBanLong = giaBanLong;
	}
	
}
